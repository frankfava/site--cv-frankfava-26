...
export function searchIndexFor(indexes: Record<string, SearchIndexEntry>, entry?: BlueprintEntry): SearchIndexEntry | undefined {
	const search = entry?.config?.search;
	if (!search?.index || search.enabled === false) return undefined;

	const index = indexes[search.index];
...
export function pagesInIndex(blueprints: BlueprintEntry[], indexes: Record<string, SearchIndexEntry>, slug: string): BlueprintEntry[] {
	return blueprints.filter((entry) => searchIndexFor(indexes, entry)?.slug === slug);
}
